...
namespace MISA.Mshopkeeper.Controllers
{
    ...
    public class FundController : ApiController
    {
        ...
        /// <summary>
        /// Hàm lấy dữ liệu chi tiết chứng từ theo chứng từ
        /// </summary>
        /// <param name="ID"></param>
        /// <returns>DocumentDetails</returns>
        /// Tạo bởi: NBDUONG(1/7/2019)
        [HttpPost]
        [Route("documents/documentDetails/{documentID}")]
        public AjaxResult GetDocumentDetailByDocumentID(Guid documentID)
        {
            var ajaxResult = new AjaxResult();
            try
            {
                using (DocumentDetailBL documentDetailBL = new DocumentDetailBL())
                {
                    ajaxResult.Data = documentDetailBL.GetAllDocumentDetails(documentID);
                    ajaxResult.Success = true;
                }   
            }
            catch (Exception)
            {
                ajaxResult.Success = false;
                ajaxResult.Messenger = "Có lỗi xảy ra khi lấy dữ liệu chứng từ theo chứng từ. Xin vui lòng liên hệ MISA để giải quyết";
            }
            return ajaxResult;
        }
