...
class Fund {
    ...
    getDataFundFromServer() {
        ...
        var filters = thisFund.buildFilterObject();
        $.ajax({
            type: "POST",
            url: url,
            ...
            success: function (response) {
                ...
                if (response.Data.length > 0) {
                    thisFund.totalRows = response.TotalCount;
                    console.log(thisFund.totalRows);
                    fund.loadDocuments(response.Data);
                    fund.loadDocumentDetail(response.Data[0].DocumentID);
                    fund.getTotalRows();
                    fund.getTotalPages();
                    fund.setDisableButtonPaging();
                } else {
                    thisFund.totalRows = 0;
                    thisFund.totalPages = 1;
                    fund.getTotalRows();
                    fund.getTotalPages();
                }
            }
     ...
     //Hàm load dữ liệu get từ trên server
    //Tạo bởi: NBDUONG (15/5/2019)
    ...
    //Hàm lấy dữ liệu chi tiết chứng từ lấy từ trên sever
    //Tạo bởi: NBDUONG(1/7/2019)
    loadDocumentDetail(documentId) {
        let totalMoney = 0;
        documentId = $('.MISABody-part_middle-content .middle-content_table-data .table-row.choose-background').data("DocumentID");
        $('.MISABody-part_middle-content .footer-content_detail-table-data .detail-table-data_list-data').html("");
        common.callAjaxToServer("POST", '/fund/documents/documentDetails/' + documentId, null, function (result) {
            if (result.Success) {
                $('.footer-content_detail-table-data .loading').hide();
                if (result.Data.length > 0) {
                    console.log(result.Data);
                    $.each(result.Data, function (i, item) {
                        $(".footer-table-row-clone span").each(function () {
                            let fieldName = $(this).parent().attr("fieldName");
                            let dataType = $(this).parent().attr("dataType");
                            if (dataType === "number") {
                                $(this).text(item[fieldName].formatNumber());
                                $(this).data("Amount", item[fieldName]);
                                totalMoney += $(this).data("Amount");
                            } else {
                        ...
                        $('.detailTotalMoney span').text(totalMoney.formatNumber());
                    });
                } else {
                    $('.detailTotalMoney span').text(0);
                }
    ...
    loadDocumentByHotKey(rowCurrent, nextStep) {
        ...
        if (rowFocus && documentId) {
            ...
            fund.loadDocumentDetail(documentId);
        }
    }
    ...
    //Tạo bởi: NBDUONG(18/5/2019)
    validateInputData() {
        this.validateInput('input[inputType="ReceiverName"]', 100);
        this.validateInput('input[inputType="Reason"]', 255);
        this.validateInput('input[fieldName="DocumentAddress"]', 255);
        //this.validateInput('input[fieldName="DocumentCode"]', 20);
    }
    ...
    //Hàm đồng bộ chứng từ từ form Chọn hóa đơn trả nợ với form Phiếu thu/ chi
    asyncDataDocumentDetail() {
        $('#recipeFormDetail .reciptFormDetail_tableData .table-row.choose-background').each(function () {
            var currentRow = $(this);
            $('.document-data_row-clone [fieldName]').each(function () {
                let fieldName = $(this).attr("fieldName");
                let val = currentRow.find("[fieldName='" + fieldName + "']").text().trim();
                console.log(fieldName);
                console.log(val);
                $(this).val(val);
            });
            $('.formDetail-info-paydebt .formDetail_table-document-data .detail-table-data_list-data').append($('.document-data_row-clone .table-row').clone(true));
        });
    }
    ...
    asyncDataFromChooseReceiptForm() {
        ...
        thisFund.asyncDataDocumentDetail();
    }
    ...
    selectOption() {
        ...
        $('body').on('mousedown', '.select-option-menu span', function () {
            ...
            $(this).parent().hide();
            $(this).parent().next().children().attr('filterType', signal);
        });
    }
    ...
    //Hàm validate các giá trị hợp lệ cho input trong form
    //Tạo bởi: NBDUONG(27/6/2019)
    checkValidateInput() {
        $('#formDetail input[fieldName="DocumentCode"]').keydown(function () {
            $('span.error-type').remove();
            var inputVal = $(this).val();
            if ($('#formDetail').hasClass('add-pay-money')) {
                var documentPayCode = /^PC[0-9]{1,10}$/;
                if (!documentPayCode.test(inputVal)) {
                    $(this).after('<span class="error-type">Số phiếu nhập có dạng PC000001 </span>');
                } else {
                    $('span.error-type').remove();
                }
            } else if ($('#formDetail').hasClass('add-collect-money')) {
                var documentCollectCode = /^PT[0-9]{1,10}$/;
                if (!documentCollectCode.test(inputVal)) {
                    $(this).after('<span class="error-type">Số phiếu nhập có dạng PT000001 </span>');
                } else {
                    $('span.error-type').remove();
                }
            }
        ...
        $('#formDetail input[fieldName="DocumentCode"]').blur(function () {
            $('span.error-type').hide();
            var inputVal = $(this).val();
            if ($('#formDetail').hasClass('add-pay-money')) {
                var documentPayCode = /^PC[0-9]{1,10}$/;
                if (!documentPayCode.test(inputVal)) {
                    $('input[fieldName="DocumentCode"]').val($(this).attr('data-previous'));
                } else {
                    $('input[fieldName="DocumentCode"]').attr('data-previous', $(this).val());
                    $('span.error-type').hide();
                }
            } else if ($('#formDetail').hasClass('add-collect-money')) {
                var documentCollectCode = /^PT[0-9]{1,10}$/;
    ...
    filterDataByAction() {
        ...
        $(".MISABody-part_middle-content .header-table-column input").keydown(function () {
            if (event.keyCode === 13) {
                $('.middle-content_table-data .table-data_list-data').html("");
                $('.MISABody-part_middle-content .loading').show();
                fund.getDataFundFromServer();
    ...
    enableInput() {
        ...
        $('.formDetail-info-paydebt .formDetail_table-document-data .detail-table-data_list-data').html("");
    }
    ...
    getTotalPages() {
        ...
        var totalRecord = thisFund.totalRows;
        var totalPageNumber = Math.floor(totalRecord / pageSize);
        ...
        thisFund.totalPages = totalPageNumber;
        if (parseInt($('.indexPageFund').val()) > totalPageNumber) {
            $('.indexPageFund').val(totalPageNumber);
        }
        $(".totalPages").text(totalPageNumber);
    ...
    //Tạo bởi: NBDUONG(22/6/2019)
    getTotalRows() {
        $(".totalDocuments").text(thisFund.totalRows);
        ...
        if (thisFund.totalRows < optionVal) {
            lastRowIndex = thisFund.totalRows;
        }
        if (lastRowIndex === 0) {
            firstRowIndex = 0;
        }
    ...
    getAutoDocumentCollectCode() {
        common.callAjaxToServer("GET", "/fund/document/getAutoCollectCode", null, function (result) {
            ...
            $('#formDetail input[fieldName="DocumentCode"]').attr("data-previous", result.Data);
        });
    }
    ...
    //Build câu lệnh filter
    //Tạo bởi: NBDUONG(26/6/2019)
    buildFilterObject() {
        var filters = [];
        var filterInputElements = $('[elementType="filterInput"]');
        
        $.each(filterInputElements, function (index, item) {
            var filterItem = {},
                field = $(item).attr('fieldData'),
                filterType = $(item).attr('filterType'),
                dataType = $(item).attr('dataType'),
                valueFilter = item.value;
            filterItem['Field'] = field;
            filterItem['Type'] = filterType ? filterType : '*';
            filterItem['DataType'] = dataType ? dataType : 'string';
            filterItem['Value'] = valueFilter;
            if (dataType === "number") {
                filterItem['Value'] = valueFilter.formatToNumber();
            }
            else if (dataType === "date") {
                filterItem["Value"] = common.convertStringJSToStringCsharp(this);
            }
